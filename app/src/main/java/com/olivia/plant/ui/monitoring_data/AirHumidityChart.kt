...
class AirHumidityChart(context: Context) : VizContainerView(context) {

    private val chart: Chart<AirHumidityData> = chart(airHumidityData) {
        size = Size(vizSize, vizSize)
        title = "Data Sensor Kelembaban Udara"

        val hour = discrete({ domain.hour }) {
            name = "Jam Data Diambil"
        }
        val airHumidity = quantitative({ domain.airHumidity }) {
            name = "Kelembaban Udara"
        }

        line(hour, airHumidity) {
...
@Serializable
data class AirHumidityData(val hour: String, val airHumidity: Double) {
    companion object {
        fun fromJson(jsonString: String): List<AirHumidityData> {
            ...
            return dataList.map { element ->
                ...
                val airHumidity = element.jsonObject["air_humidity"]?.jsonPrimitive?.double
                ...
                AirHumidityData(hour, airHumidity!!)
            }
        }
    }
}

val airHumidityData = AirHumidityData.fromJson(jsonString)
